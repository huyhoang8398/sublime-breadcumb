...
try:
    ...
    print("[Breadcrumb] LSP package not found — plugin disabled.")
    _LSP_AVAILABLE = False


if _LSP_AVAILABLE:
    _context = []  # current symbol path
    _breadcrumb = ""  # file path breadcrumb
            ...
            try:
                ...
                for s in sessions:
                    if s.has_capability("documentSymbolProvider"):
                        if s.config.selector and view.match_selector(
                            0, s.config.selector
                        ):
                            session = s
                            break
...
def plugin_loaded():
    if _LSP_AVAILABLE:
        print("[Breadcrumb] Plugin loaded")
    # else: already printed the warning at import time
